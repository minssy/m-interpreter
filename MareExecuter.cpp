...
/**  */
void 
MareExecuter::setPropertyRun(CodeSet const& varCode, SymKind sk)
{
    auto symPt = symTablePt(varCode);
    int varAdrs = symPt->adrs;
    int diffLength = 1;
    ...
    case Insert: // 작업필요
    {
        ...
        if (vo.getType() == OBJECT_T) {
            ...
            // }
            int lth = symPt->frame;
            DynamicMem.updateInsert(varAdrs + idx * lth, diffLength * lth, varAdrs + symPt->args -1, tmpStructObj, lth);
